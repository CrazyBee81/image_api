...
describe('Test if files are been processed and delivered', () => {
    ...
    it('an image gets processed', async () => {
            const transformed = await transformeImage(testInput.height, testInput.width, testInput.fileDirectory, testInput.inputFile, testInput.outputFile);
            expect(transformed).toBeTruthy();
        }
    )
    it('an image has been safed', async () => {
            const exists = await checkFileExists(testInput.fileDirectory, testInput.outputFile);
             expect(exists).toBeTruthy();
        }
    )
    // Teardown test image
    afterAll(async () => {
            await fsPromises.unlink(`${testInput.fileDirectory}/${testInput.outputFile}`);
        }
    );
...
describe('Test error handling of transformer', () => {
    it('returns false if image has not been transformed ', async () => {
        const transformed = await transformeImage(testInput.height, -1, "test", testInput.inputFile, "test");
